...
export class FeedbackAssessmentFormComponent implements OnInit, OnDestroy {
  ...
  protected loadRelationshipsOptions(): void {
    this.isLoading = true;
    forkJoin({
      responder: this.feedbackResponderService
        .find(this.responderId!)
        .pipe(map((res: HttpResponse<IFeedbackResponder>) => res.body ?? null)),
  ...
  private loadDraft(): void {
    this.localStorageService.getDraft(this.responderId!).subscribe({
      next: response => {
  ...
  saveDraft(): void {
    this.localStorageService.saveDraft(this.responderId!, this.requestForm.value).subscribe({
  ...
  submit() {
    ...
    const teachOtherSubmission = this.teachOtherService.create(
      this.teachOtherFormService.transformToNewTechOther(this.teachOtherForm, this.feedbackResponder!),
    );

    // Combine all submissions
    const submissions = [...feedbackSubmissions, ...skillSubmissions, teachOtherSubmission];
  ...
  updateResponderStatus(responderId: number, responderStatus: string) {
    try {
      this.feedbackResponderService.updateResponderStatus(responderId, responderStatus).subscribe(res => {
        if (res) {
          console.log(res);
        }
      });
