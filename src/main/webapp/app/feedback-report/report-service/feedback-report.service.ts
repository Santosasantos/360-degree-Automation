...
export class FeedbackReportService {
  ...
  findByPin(pin: string): Observable<EntityResponseTypeforEmployee> {
    return this.http.get<EntityResponseTypeforEmployee>(`${this.resourceUrlforemployees}/pin/${pin}`);
  }

  fetchRatingfromOthers(pin: string, feedbacksubname: string, year: number): Observable<number> {
    const params = new HttpParams().set('pin', pin).set('feedbacksubname', feedbacksubname).set('year', year);
    return this.http.get<number>(`${this.resourceUrlforfeedbackdetails}/fetchratingfromothers`, { params }).pipe(map(res => res ?? null));
  }

  fetchRatingforSelf(pin: string, feedbacksubname: string, year: number): Observable<number> {
    const params = new HttpParams().set('pin', pin).set('feedbacksubname', feedbacksubname).set('year', year);
    return this.http.get<number>(`${this.resourceUrlforfeedbackdetails}/fetchratingforself`, { params }).pipe(map(res => res ?? null));
  }
