...
export class FeedbackReportComponent {
  ...
  getRatings(yearname: number): void {
    ...
    this.feedbackSubTypes?.forEach(feedbacksub => {
      ...
      const othersPromise = this.feedbackReportService
        .fetchRatingfromOthers('6304', feedbacksubname, yearname)
      ...
      const selfPromise = this.feedbackReportService
        .fetchRatingforSelf('6304', feedbacksubname, yearname)
        .pipe(catchError(() => of(0)))
        .toPromise()
        .then(rating => {
  ...
  checkForRatings(): boolean {
    return this.feedbackratingsforothers.some(rating => rating > 0) && this.feedbackratinsforself.some(rating => rating > 0);
  }
  ...
  getRating(ratings: number[], feedbackTypeIndex: number, subtypeIndex: number): number {
    const startIndex = this.feedbackTypesSharedCollection
      .slice(0, feedbackTypeIndex)
