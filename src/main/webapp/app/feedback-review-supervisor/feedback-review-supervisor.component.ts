...
export class FeedbackReviewSupervisorComponent implements OnChanges, OnDestroy {
  ...
  loadInfo(): void {
    ...
    forkJoin({
      responders: this.feedbackResponderService
        .findFeedbackResponders(this.feedbackId)
      ...
      extraquestions: this.extraQuestionService
        .findByFeedbackId(this.feedbackId)
  ...
  private loadEmployeeReplacements(responder: IFeedbackResponder): void {
    ...
    this.feedbackService
      .searchEmployees(responder.category!, responder.feedback.createdBy)
      .pipe(
        map(response => response.body ?? []),
        map(employees => employees.filter(emp => !this.selectedresponders.some(r => r.employee?.id === emp.id))),
    ...
    try {
      ...
      const updatePromises = this.selectedresponders.map(responder =>
        this.feedbackResponderService
          .updateResponderStatus(responder.id!, status)
          .pipe(
  ...
  canConfirmReplacement(): boolean {
    return this.replacementType === 'STAKEHOLDER' ? !!this.replacementEmail : !!this.selectedReplacement;
  }
